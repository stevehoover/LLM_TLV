...
function run_sandpiper() {
  ...
  # Depending on version and argument shape, SandPiper may ignore unknown
  # command-line arguments with only a warning (observed with bare tokens and
  # --iArgs on SandPiper 1.14-2022/10/10). In this flow the arguments come from
  # config.json's M5_configs, so an ignored argument always means a malformed
  # config, and silently proceeding degrades multi-config FEV to the default
  # config only. Treat it as fatal regardless of SandPiper's exit status.
  if grep -q "Unrecognized command-line arguments ignored" "${TEMP_DIR}/sandpiper_${file}${config_suffix}.log" 2>/dev/null; then
    echo
    echo "SandPiper log for ${file}.tlv -> ${sv_file}:"
    echo
    cat "${TEMP_DIR}/sandpiper_${file}${config_suffix}.log"
    echo
    fail 2 "SandPiper ignored unrecognized command-line arguments for config '${config_desc}'. These arguments come from M5_configs in config.json, so this always indicates a malformed config value. Fix the M5_configs entry (each value must be a string of valid sandpiper-saas arguments, e.g. \"--m5def cond_w_1=0\")."
  fi
  if [[ ! -f ${sv_file} ]]; then
...
if [[ $NEED_FULL_FEV == false || $MISSING_SV == true ]]; then
  ...
  # Guard: M5_configs must be an object mapping config names to plain strings
  # of sandpiper-saas arguments. Any other shape (an array, nested objects)
  # gets shredded by the to_entries expression below into stray tokens that
  # some SandPiper versions ignore with only a warning, which silently
  # degrades multi-config FEV to the default config only. Fail loudly here
  # before any command line is built.
  if jq -e 'has("M5_configs")' config.json > /dev/null 2>&1; then
    if ! jq -e '.M5_configs | type == "object" and (to_entries | all(.value | type == "string"))' config.json > /dev/null 2>&1; then
      fail 2 "Malformed M5_configs in config.json: it must be an OBJECT mapping config names to STRINGS of sandpiper-saas arguments, e.g. {\"W_4\": \"--m5def cond_w_1=0\"}. Fix config.json and rerun."
    fi
  fi
